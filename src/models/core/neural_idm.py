...
class NeurIDMModel(AbstractModel):
    ...
    def call(self, inputs):
        ...
        _, displacement_seq, action_seq, _ = self.forward_sim.rollout([\
...
class IDMForwardSim(tf.keras.Model):
    ...
    def idm_driver(self, idm_state, idm_params):
        ...
        desired_gap = min_jamx + tf.relu(_gap)
    ...
    def rollout(self, inputs):
        ...
        for step in range(1, self.rollout_len+1):
            ...
            if step-1 == 0:
                ...
                em_delta_x_seq = tf.concat([em_delta_x_seq, em_delta_x], axis=1)

            displacement_seq = tf.concat([displacement_seq, displacement], axis=1)

        return em_delta_x_seq, displacement_seq, act_seq, [f_att_seq, m_att_seq]
